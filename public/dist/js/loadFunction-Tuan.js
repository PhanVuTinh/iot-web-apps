...
var user_ID = 0;
var socket_global = '';
var ispageNotification = false;
/*Biến xác định xem có phải đang ở trang xem thông báo hay không
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data.Items != null) {
                resultdata.data.Items.forEach(function(items) {
                    ...
                    html += "<span class='fa fa-pencil-square-o fa-2x'></a>&nbsp";
...
//
function getListUser3(conf, token, secu, index, size) {
    // showModalNoti(conf,token,secu,threshold_level,datatype_name,datatype_id,data_value,notif_title,stationid,datecreated,threshold_message,notif_id)
    var html = "";
    var html2 = "";
    // alert("vô");
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data.Items != null) {
                resultdata.data.Items.forEach(function(items) {
                    ...
                    html += '<td><table><tr><td>Ngày sinh: </td><td>' + moment(items.user_birthday).format('DD/MM/YYYY') + '</td></tr><tr><td>Số điện thoại:&nbsp  </td><td>' + items.user_phone + '</td></tr><tr><td>Email: </td><td>' + items.user_email + ' </td></tr></table></td>';
                    //Ngày sinh: '+ moment(items.user_birthday).format('DD/MM/YYYY') + '<br>Số điện thoại: ' + items.user_phone + '<br>Email: ' + items.user_email + '</td>';
                    if (items.user_lockStatus) { html += "<td><a onclick='confirm_edit()'  title='Mở khoá' href='/quantrac/quantri/nguoidung/mokhoa/" + items.user_id + " ' ><span><i class='glyphicon glyphicon-ok-sign' ></i></span></a></td>"; } else { html += "<td><a onclick='confirm_edit()' title='Khoá' href='/quantrac/quantri/nguoidung/khoa/" + items.user_id + " ' ><span><i class='glyphicon glyphicon-remove-sign' ></i></span></a></td>" };
                    //html += '<td><span>Khoá</span></td>' +} else { +'<span>Mở</span>'+ } + '</td>';
                    html += "<td><a title='Cập nhật thông tin người dùng' href='/quantrac/quantri/nguoidung/sua/" + items.user_id + "'>";
                    html += "<span class='fa fa-pencil-square-o fa-2x'></a>&nbsp";
                    //html += "<a title='Xoá thông tin người dùng' href='/quantrac/nguoiquantri/nguoidung/xoa/"+ items.user_id +"'>"; 
                    //html += "<span class='glyphicon glyphicon-trash'></a>&nbsp";
                    //if(items.user_lockStatus){html += "<a title='Mở khoá' href='/nguoiquantri/nguoidung/mokhoa/"+items.user_id + " ' ><span><i class='glyphicon glyphicon-ok-sign' onclick='success()''></i></span></a></td>";}else{html += "<a title='Khoá' href='/quantrac/nguoiquantri/nguoidung/khoa/"+items.user_id +"  ' ><span><i class='glyphicon glyphicon-remove-sign' onclick='success()'></i></span></a></td>"};
                ...
                $("#hienthidsnguoidungsupad").html(html);
            }
            for (i = 1; i <= totals; i++) {
                html2 += '<li><a href="#" onclick="processPaginationUser3(' + "'" + conf + "'" + ',' + "'" + token + "'" + ',' + "'" + secu + "'" + ',' + index + ',' + pagesize + ');return false;">' + i + '</a></li>';
...
function processPaginationUser3(conf, token, secu, index, pagesize) {
    var selector = '.pagi-custom li';
    $(selector).on('click', function() {
        $(selector).removeClass('active');
        $(this).addClass('active');
        ...
        var page = parseInt($("ul.pagi-custom .active a").text()); /*Lấy số ở trên pagination phân trang chuyển thành số*/
        pagenoti = (page - 1) * pages;
        getListUser3(conf, token, secu, (page - 1), pages);
    });
}
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data.Items != null) {
                resultdata.data.Items.forEach(function(items) {
                    ...
                    html += "<span class='fa fa-pencil-square-o fa-2x'></a>";
                    html += "<a title='Xoá thông tin người dùng' href='/quantrac/nguoiquantri/nguoidung/xoa/" + items.user_id + "'>";
                    html += "<span class='fa fa-times fa-2x'></a>";
                    html += "<a title='Xoá thông tin người dùng' href='/quantrac/nguoiquantri/nguoidung/xoa/" + items.user_id + "'>";
                    html += "<span class='fa fa-times fa-2x'></a></td>";
                    //html +=  + if(items.user_lockStatus) { +'<a href="/nguoiquantri/nguoidung/mokhoa/<%= rs.user_id %>" title="Mở khoá" ><span><i class="glyphicon glyphicon-ok-sign" onclick="success()"></i></span></a>' +} else { +'<a href="/nguoiquantri/nguoidung/khoa/<%= rs.user_id %>" title="Khoá" ><span><i class="glyphicon glyphicon-remove-sign" onclick="success()"></i></span></a>'+ } + '</td>';
                    html += '</tr>';
                });
            ...
            if (resultdata.data.Items != null) {
                resultdata.data.Items.forEach(function(items) {
                    if (items.feedback_answerContent != null) {
                        ...
                        html += '<span><i class="fa fa-comments fa-2x" aria-hidden="true"></i></span></a>&nbsp';
                        ...
                        html += "<span class='fa fa-trash fa-2x'></a></td>";
                        html += '</tr>';
                    } else {
                        ...
                        html += "<span><i class='fa fa-comments fa-2x' ></i></span></a>&nbsp";
                        html += "<a onclick='return confirm_delete()' title='Xoá thông tin phản hồi' href='/quantrac/nguoiquantri/phanhoi/xoa/" + items.feedback_id + "'>";
                        html += "<span class='fa fa-trash fa-2x'></a></td>";
                        html += '</tr>';
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data.Items != null) {
                resultdata.data.Items.forEach(function(items) {
                    ...
                    html += "<span><i class='fa fa-plus-square fa-2x' ></i></span></a>&nbsp";
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data.Items != null) {
                resultdata.data.Items.forEach(function(items) {
                    ...
                    html += "<span class='fa fa-plus-square fa-2x'></a>&nbsp";
                    ...
                    html += '<i class="fa fa-list-alt fa-2x" aria-hidden="true"></i></a></td>';
                    html += '</tr>';
                });
...
function loadStokingforGuest(id, config, token, security, callback) {
    ...
    queue2.push(1);
    jQuery.ajax({
        url: config + '/api/stocking/getbyguestuser/4',
        type: 'GET',
        contentType: 'application/json',
        headers: {
            'authorization': token,
        ...
        success: function(resultdata) {
            // console.log('bạn Tuấn lỗi chỗ này nha! Dân đã fixed'+ JSON.stringify(resultdata));
            var StockingArr = resultdata.data;
            StockingArr.forEach(function(data, index) {
...
function loadHarvestforGuest(conf, token, secu, id) {
    ...
    if (stockingId != -1) {
        jQuery.ajax({
            url: conf + '/api/harvest//getbystocking/' + stockingId,
            type: 'GET',
            contentType: 'application/json; charset=utf-8',
            headers: {
                'authorization': token,
            ...
            success: function(resultdata) {
                ...
                var harvestArray = resultdata.data;
                if (harvestArray != null) {
                    harvestArray.forEach(function(item, index) {
                        $('#harvest_id').append($("<option></option>").attr("value", item.harvest_id).text(moment(item.harvest_harvestDate).format('DD/MM/YYYY')));
                        $("#harvest_id").css("display", "block");
                    });
                } else {
                    displayError("Vụ nuôi chưa thu hoạch");
                }
...
/*HÀM LIÊN QUAN TỚI */
/*HÀM lẤY danh sách */
function getPostforGuest(index, size, conf) {
    // showModalNoti(conf,token,secu,threshold_level,datatype_name,datatype_id,data_value,notif_title,stationid,datecreated,threshold_message,notif_id)
    var html = "";
    var html2 = "";
    var html3 = "";
    ...
    jQuery.ajax({
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data.Items != null) {
                ...
                resultdata.data.Items.forEach(function(items) {
                    if (resultdata.data.Items.postcate_id != 27) {
                        ...
                        //if (items.postcate_id != 27) {
                        html += '<div class=" col-sm-4 " style="height: 700px;">';
                        html += '<div class="well" style="height: 700px;">'
                        html += '<div class=" blog-post blog-large wow fadeInLeft" data-wow-duration="300ms" data-wow-delay="0ms" style="border: 0px; ">';
                        html += '<article>';
                        html += '<header class="entry-header">';
                        html += '<div class="entry-thumbnail" style="height: 300px;">';
                        html += '<img class="img-responsive" src="' + items.post_smallPicture + '" width="100%" height="500px" style="height:250px" >';
                        html += '</div>';
                        ...
                        html += '<div class="col-sm-4" style="height: 750px;padding: 10px 10px ;">';
                        ...
                        html += '<img class="img-responsive" src="/images/product.jpg" alt="" height="100%" width="100%" >';
                        html += '</div>';
                        html += '<div class="entry-date">' + moment(items.post_createDate).format('DD/MM/YYYY') + '</div>';
            ...
            for (i = 1; i <= totals; i++) {
                html2 += '<li><a href="#" onclick="processPaginationPPost(' + (i - 1) + ',' + pagesize + ');return false;">' + i + '</a></li>';
...
function getFoodforGuest(index, size, conf) {
    // showModalNoti(conf,token,secu,threshold_level,datatype_name,datatype_id,data_value,notif_title,stationid,datecreated,threshold_message,notif_id)
    var html = "";
    var html2 = "";
    var html3 = "";
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data != null) {
                ...
                resultdata.data.forEach(function(items) {
                    ...
                    html += '<header class="entry-header">';
                    html += '<div class="entry-thumbnail" style="height: 300px;">';
                    html += '<img class="img-responsive" src="' + items.post_smallPicture + '" width="100%" height="100%" style="height:300px" > ';
                    html += '</div>';
                    html += '<div class="entry-date">' + moment(items.post_createDate).format('DD/MM/YYYY') + '</div>';
                    html += '<h2 class="entry-title"><a href="/tintuc/chitietbaiviet/' + items.post_id + '">' + items.post_title + '</a></h2>';
                    html += '</header>';
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data != null) {
                ...
                resultdata.data.forEach(function(items) {
                    ...
                    html += '<div class="col-sm-4" style="height: 500px;padding: 10px 10px; background-color: #fff; border-width: 1px; boder-color: #6A5858">';
                    html += '<div class=" blog-post blog-large wow fadeInLeft" data-wow-duration="300ms" data-wow-delay="0ms" style="border: 0px; margin: 15px">';
                    html += '<article>';
                    html += '<header class="entry-header">';
                    html += '<div class="entry-thumbnail">';
                    html += '<img class="img-responsive" src="' + items.post_smallPicture + '" alt="" height="250px" width="300px" style="height:250px;width=300px">';
                    html += '</div>';
                    html += '<div class="entry-date" style="font-size:14px">' + moment(items.post_createDate).format('h:mm A DD/MM/YYYY') + '</div>';
                    html += '<h2 class="entry-title"><a href="#">' + items.post_title + '</a></h2>';
                    html += '</header>';
                    html += '<div class="entry-content">';
                    //html += '<p>'+items.post_description.substr(0,200)+' ...</p>';
                    html += '<a class="btn btn-primary" href="/tintuc/chitietbaiviet/' + items.post_id + '">Xem tin</a>';
                    html += '</div>';
                    html += '</article>';
                    html += '</div>';
                    html += '</div>';
                    html3 += '<a href="/khachhang/tintuc/chitietbaiviet/' + items.post_id + '">' + items.post_title + '<br>' + moment(items.post_createDate).format('h:mm A DD/MM/YYYY') + '</a><br>';
                });
...
/*Xử lý phân trang cho productcategory*/
function processPaginationPPost(index, pagesize) {
    var selector = '.pagi-custom li';
    $(selector).on('click', function() {
        $(selector).removeClass('active');
        $(this).addClass('active');
        var page = parseInt($("ul.pagi-custom .active a").text()); /*Lấy số ở trên pagination phân trang chuyển thành số*/
        // pagenoti = (page - 1) * pages;
        getPostforGuest(index, pagesize, conf);
    });
}
    ...
    jQuery.ajax({
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data != null) {
                ...
                // html += '<div id="wrap-qr"><img id="qrious" ></div>';
                // html += '<img class="img-responsive" src="/images/qrcode.png" alt="">';
                $("#hienthi").html(html);
            }
            for (i = 1; i <= totals; i++) {
                html2 += '<li><a href="#" onclick="processPaginationPPost(' + (i - 1) + ',' + pagesize + ');return false;">' + i + '</a></li>';
        ...
        success: function(resultdata) {
            ...
            if (resultdata.data.Items != null) {
                resultdata.data.Items.forEach(function(items) {
                    ...
                    html += '<div id="ans' + items.comment_id + '" class="collapse">';
                    html += '<div class="well">';
            ...
            for (i = 1; i <= totals; i++) {
                html2 += '<li><a href="#" onclick="processPaginationPPost("<%=conf%>",' + (i - 1) + ',' + pagesize + ');return false;">' + i + '</a></li>';
...
io.sockets.on('connection', function(socket) {
    ...
    socket.on("sendPhoto", function(data) {
        var guess = data.base64.match(/^data:image\/(png|jpg);base64,/)[1];
        var ext = "";
        switch (guess) {
            case "png":
                ext = ".png";
