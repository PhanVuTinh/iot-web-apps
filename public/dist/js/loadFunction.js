...
function loadFIRST(conf, token, secu, userid, sock, ishompage_loaddata) {
    console.log("token: " + token + ", secu: " + secu + ", config: " + config);

    // if (typeof(Storage) !== "undefined") {
    // 	sessionStorage.apiConf = conf;
    // 	sessionStorage.apiToken = token;
    // 	sessionStorage.apiSecu = secu;
...
function getListStationPaginationByUserId(conf, token, secu, userid, index, pagesize) {
    ...
    var station_address = '',
        ...
        station_location = '';
    jQuery.ajax({
        url: conf + '/api/station/getpagination/' + userid + '?page=' + index + '&pageSize=' + pagesize + '&keyword=' + keyword,
        type: 'GET',
        headers: { 'Authorization': secu + token },
        contentType: 'application/json; charset=utf-8',
        success: function(resultdata) {
            if (resultdata.data.Items.length > 0) {
                ...
                resultdata.data.Items.forEach(function(items) {
                    if (!items.isDelete) {
                        ...
                        html += "<span class='fa fa-edit fa-2x'></a></td>";

                        html += '<td><a id="btn_xoa" title="Xóa thông tin trạm" href="#" onclick="eventDeleteStation(' + "'" + conf + "'" + ',' + "'" + token + "'" + ',' + "'" + secu + "'" + ',' + items.station_id + ',' + "'" + items.station_name + "'" + ',' + "'" + items.region_id + "'" + ')">';
...
/*Hàm lấy danh sách trạm theo userid cho trang danh sách trạm sau xóa*/
function getListStationPaginationByUserIdAfterDelete(conf, token, secu, userid, index, pagesize) {
    var totals = 0;
    var html = "";
    var html2 = "";
    var keyword = '';
...
/*hàm xóa trạm*/
function eventDeleteStation(conf, token, secu, stationid, stationname, regionid) {
    var selector = '#btn_xoa';
    var stationname = JSON.stringify(stationname);
    if (confirm("Bạn có chắc chắn muốn xóa?") == true) {
        jQuery.ajax({
            url: conf + '/api/station/updateafter/' + stationid,
            type: 'PUT',
            dataTypes: 'json',
            contentType: 'application/json; charset=utf-8',
            headers: {
...
/*Hàm lấy danh sách trạm điều hành*/
...
/*hàm xóa pond*/
function eventDeletePond(conf, token, secu, pondid, userid, regionid) {
    var selector = '#btn_xoa';
    if (confirm("Bạn có chắc chắn muốn xóa?") == true) {
        jQuery.ajax({
            url: conf + '/api/pond/updateafter/' + pondid,
            type: 'PUT',
            dataTypes: 'json',
            contentType: 'application/json; charset=utf-8',
            headers: {
...
/*HÀM lấy danh sách AO sau khi xóa <Hoa>*/
function getListPondbyUserIdAfterDelete(conf, token, secu, userid, index, pagesize) {
    var keyword = "";
    var totals = "";
    var html = "";
    var html2 = "";
            ...
            if (resultdata.data.Items.length > 0) {
                ...
                resultdata.data.Items.forEach(function(items) {
                    if (!items.isDelete) {
                        ...
                        html += '<td><a id="btn_xoa" title="Xóa thông tin trạm điều hành" href="#" onclick="eventDeleteSink(' + "'" + conf + "'" + ',' + "'" + token + "'" + ',' + "'" + secu + "'" + ',' + items.sink_id + ',' + "'" + items.sink_name + "'" + ',' + "'" + items.region_id + "'" + ')">';
                        html += "<span class='fa fa-trash fa-2x'></a></td>";

                        html += '<td><Button onclick="getListStationBySinkId(' + "'" + conf + "'" + ',' + "'" + token + "'" + ',' + "'" + secu + "'" + ',' + items.sink_id + ',' + "'" + items.sink_name + "'" + ')">';
                        html += '<i class="fa fa-list-alt fa-2x" aria-hidden="true"></i></Button></td>';
                        html += '</tr>';
                    }
                });
...
/*HÀm cho trạm điều hướng sau khi xóa*/
function getListSinkAfterDelete(conf, token, secu, userid, index, size) {
    var html = "";
    var html2 = "";
    var totals = 0;
    var keyword = "";
...
/*hàm xóa sink*/
function eventDeleteSink(conf, token, secu, sinkid, sinkname, regionid) {
    var selector = '#btn_xoa';
    var sinkname = JSON.stringify(sinkname);
    if (confirm("Bạn có chắc chắn muốn xóa?") == true) {
        jQuery.ajax({
            url: conf + '/api/sink/updateafter/' + sinkid,
            type: 'PUT',
            dataTypes: 'json',
            contentType: 'application/json; charset=utf-8',
            headers: {
...
function getListRegionByUserId(conf, token, secu, userid, index, pagesize) {
    ...
    jQuery.ajax({
        url: conf + '/api/region/getpagination/' + userid + '?page=' + index + '&pageSize=' + pagesize + '&keyword=' + keyword,
        type: 'GET',
        headers: { 'Authorization': secu + token },
        contentType: 'application/json; charset=utf-8',
        success: function(resultdata) {
    ...
    if (typeof(Storage) !== "undefined") {
        if (sessionStorage.stationconfig) {
            ...
            stationconfig.data.forEach(function(items) {
                ...
                html += '<span class="fa fa-pencil-square-o fa-2x"></span></a>';
                html += '<a style="margin-left:8px" href="" onclick="deleteSetUpStationConfig(' + stt + ');return false;">';
                html += '<span class="fa fa-times fa-2x"></span></a></td>';
                html += '</tr>';
                stt++;
            });
            if (html != '') {
...
function setUpOneConfig() {
    if ($('#frmTramCauHinh').valid()) {
        ...
        if (arrayConfigTypeId.indexOf($('#configtype_id').val()) == -1) {
            ...
            var stationconfig_createDate = moment().format('DD-MM-YYYY, HH:mm');
            station_config = {
                station_config_stt: stationconfig.stt,
                /*Mởi thêm*/
                configtype_id: $('#configtype_id').val(),
                configtype_name: $('#configtype_id :selected').text(),
                /*Lấy tên của loại cấu hình để hiển thị*/
                stationconfig_value: $('#stationconfig_value').val(),
            }
...
function deleteSetUpStationConfig(itemsDelete) {
    ...
    if (stationconfig.stt > 0) {
        stationconfig.data.forEach(function(items) {
            ...
            htmlDelete += '<span class="fa fa-pencil-square-o fa-2x"></span></a>';
            htmlDelete += '<a style="margin-left:8px" href="" onclick="deleteSetUpStationConfig(' + stt + ');return false;">';
            htmlDelete += '<span class="fa fa-times fa-2x"></span></a></td>';
            htmlDelete += '</tr>';
            stt++;
        });
...
function saveSetUpStationConfig() {
    if ($('#frmCNTramCauHinh').valid()) {
        ...
        var stationconfig_createDate = moment().format('DD-MM-YYYY, HH:mm');
        station_config = {
            station_config_stt: $("#station_config_stt").val(),
            /*Mởi thêm*/
            configtype_id: $('#configtypeid').val(),
            configtype_name: $('#configtypeid :selected').text(),
            /*Lấy tên của loại cấu hình để hiển thị*/
        ...
        stationconfig.data.forEach(function(items) {
            ...
            htmlUpdate += '<span class="fa fa-pencil-square-o fa-2x"></span></a>';
            htmlUpdate += '<a style="margin-left:8px" href="" onclick="deleteSetUpStationConfig(' + stt + ');return false;">';
            htmlUpdate += '<span class="fa fa-times fa-2x"></span></a></td>';
            htmlUpdate += '</tr>';
            stt++;
        });
            ...
            if (resultdata.data.Items.length > 0) {
                ...
                resultdata.data.Items.forEach(function(items) {
                    if (!items.isDelete) {
                        ...
                        html += "<span class='fa fa-pencil-square-o fa-2x'></a></td>";
                        html += '<td><a id="btn_xoa" title="Xóa thông tin trạm sông" href="#" onclick="eventDeleteRiver(' + "'" + conf + "'" + ',' + "'" + token + "'" + ',' + "'" + secu + "'" + ',' + items.river_id + ',' + "'" + items.river_name + "'" + ',' + "'" + items.region_id + "'" + ')">';
                        html += "<span class='fa fa-trash fa-2x'></a></td>";
                        html += "</tr>";
                    }
                });
...
//ham lay ve danh sach trạm sông sau xóa <Hoa>
function getListRiverPaginationAfterDelete(conf, token, secu, userid, index, pagesize) {
    var totals = 0;
    var html = "";
    var html2 = "";
    var keyword = "";
...
/*hàm xóa trạm sông*/
function eventDeleteRiver(conf, token, secu, riverid, rivername, regionid) {
    var selector = '#btn_xoa';
    var rivername = JSON.stringify(rivername);
    if (confirm("Bạn có chắc chắn muốn xóa?") == true) {
        jQuery.ajax({
            url: conf + '/api/river/updateafter/' + riverid,
            type: 'PUT',
            dataTypes: 'json',
            contentType: 'application/json; charset=utf-8',
            headers: {
