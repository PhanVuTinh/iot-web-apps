...
regionController.get('/themvung', service.ensureAuthenticated, function(req, res) {
    res.render("superadmin/region/themvung", { title: 'Thêm vùng mới', secu: config.securitycode, conf: config.urladdress, token: req.session.token, userid: req.session.userid, username: req.session.username, fullName: req.session.fullName });
});
regionController.post('/themvung', function(req, res) {
    ...
    service.post(options, function(error, body) {
        ...
        console.log(body);
        res.redirect('/quantrac/quantri/region/danhsachvung');
    });

});
regionController.get('/danhsachvung', service.ensureAuthenticated, function(req, res) {

    res.render("superadmin/region/danhsachvung", {
...
regionController.get('/capnhatvung/:id', service.ensureAuthenticated, function(req, res) {
    ...
    service.get(options, function(error, data) {
        if (error) {
            ...
            res.render("superadmin/region/capnhatvung", { title: 'Cập nhật vùng mới', regData: regionData, regid: id, wardid: regionData.ward_id, secu: config.securitycode, conf: config.urladdress, token: req.session.token, userid: req.session.userid, username: req.session.username, fullName: req.session.fullName });
        }
    });
});
