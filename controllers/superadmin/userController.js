...
userController.get('/taonguoidung', function(req, res) {
    ...
    res.render("superadmin/user/createUser");
});

userController.get("/danhsach", service.ensureAuthenticated, function(req, res) {
    res.render("superadmin/user/ListUser", { title: 'Xem danh sách người dùng', secu: config.securitycode, conf: config.urladdress, token: req.session.token, userid: req.session.userid, username: req.session.username, fullName: req.session.fullName });
});
...
userController.get('/sua/:id', service.ensureAuthenticated, function(req, res) {
    ...
    service.get(options, function(error, data) {
        if (error) {
            ...
            res.render('superadmin/user/updateuser', { title: 'Sửa thông tin người dùng', moment: moment, users: userData, secu: config.securitycode, conf: config.urladdress, token: req.session.token, userid: req.session.userid, username: req.session.username, fullName: req.session.fullName });
        }
    });
});
