...
sinkController.post('/themtramdieuhanh',service.ensureAuthenticated,function(req,res){
	...
	var sink_latitude = req.body.sink_latitude;
	var sink_longitude = req.body.sink_longitude;
	var sink_location = service.latLngToDMS(sink_latitude)+((sink_latitude > 0)? 'N': 'S')+' ,'+ service.latLngToDMS(sink_longitude)+((sink_longitude > 0)? 'E': 'W');
	...
	var options = {
		...
		form:{
			...
			sink_latitude: sink_latitude,
			sink_longitude: sink_longitude,
		}
	};
...
sinkController.get('/lichsutramdieuhanh',service.ensureAuthenticated,function(req,res){
	res.render("manager/tramdieuhanh/lichsutramdieuhanh",{title:"Lịch sử trạm điều hành",secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName});
});
