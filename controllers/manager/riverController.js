...
riverController.post('/themtramsong',service.ensureAuthenticated,function(req,res){
	...
	var river_latitude = req.body.river_latitude;
	var river_longitude = req.body.river_longitude; 
	var river_location = service.latLngToDMS(river_latitude)+((river_latitude > 0)? 'N': 'S')+' ,'+ service.latLngToDMS(river_longitude)+((river_longitude > 0)? 'E': 'W');
	...
	var options = {
		...
		form:{
			...
			river_latitude: river_latitude,
			river_longitude: river_longitude, 
		}
	};
...
riverController.get('/lichsutramsong',service.ensureAuthenticated,function(req,res){
	res.render("manager/song/lichsutramsong",{title:"Lịch sử trạm sông",secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName});
});
