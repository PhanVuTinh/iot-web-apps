...
stationController.post('/themtram',service.ensureAuthenticated,function(req,res){
	...
	var station_latitude = req.body.station_latitude;
	var station_longitude = req.body.station_longitude;
	var station_location = service.latLngToDMS(station_latitude)+((station_latitude > 0)? 'N': 'S')+' ,'+ service.latLngToDMS(station_longitude)+((station_longitude > 0)? 'E': 'W');
...
stationController.get('/lichsutram',service.ensureAuthenticated,function(req,res){
	res.render("manager/tram/lichsutram",{title:"Lịch sử trạm",secu:config.securitycode,conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName});
});
