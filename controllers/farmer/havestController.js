        ...
        if (error) {
            ...
            if (result.Error) {
                ...
                if (req.params.stocking_id) {
                    ...
                    if (flashCheckExes) {
                        res.render('farmer/havest/addHavest', {
                            title: 'Thêm thu hoạch cho đợt nuôi số ' + req.params.stocking_id,
        ...
        if (error) {
            ...
            if (result.Error) {
                ...
                if (params.stocking_id) {
                    ...
                    if (flashCheckExes) {
                        res.render('farmer/havest/havestList', {
                            title: 'Thu hoạch của đợt nuôi số ' + params.stocking_id,
                            conf: config.urladdress,
                            token: token,
                            userId: req.user.id,
                            fullName: req.session.fullName,
                    ...
                    if (stockingOfUser.length != 0) {
                        res.render('farmer/havest/havestList', {
                            title: 'Thu hoạch của đợt nuôi số ' + stockingOfUser[0].stocking_id,
                            conf: config.urladdress,
                            token: token,
                            userId: req.user.id,
                            fullName: req.session.fullName,
                        ...
                        res.render('farmer/havest/havestList', {
                            title: 'Chưa có đợt thả nuôi nào được tạo',
                            conf: config.urladdress,
                            token: token,
                            userId: req.user.id,
                            fullName: req.session.fullName,
        ...
        if (error) {
            ...
            if (result.Error) {
                ...
                if (result.data.user_id == req.user.id) {
                    res.render('farmer/havest/updateHavest', {
                        title: 'Cập nhật đợt thu hoạch số' + result.data.harvest_id,
                        conf: config.urladdress,
                        token: token,
                        userId: req.user.id,
                        fullName: req.session.fullName,
