                ...
                service.get(options, function(error, result) {
                    if (error) {
                        ...
                        if (result.Error) {
                            ...
                            if (result.data != null) {
                                res.render('farmer/activity/addActivity', {
                                    title: 'Thêm hoạt động ao ' + result.data.pond_id,
                ...
                service.get(options, function(error, result) {
                    if (error) {
                        ...
                        if (result.Error) {
                            ...
                            if (result.data != null) {
                                res.render('farmer/activity/activityList', {
                                    title: 'Chăm sóc ao ' + result.data.pond_id,
                                    conf: config.urladdress,
                                    token: token,
                                    userId: req.user.id,
                                    fullName: req.session.fullName,
                                    ...
                                    pondOfStocking: pondOfStocking,
                                    secu: config.securitycode,
                                    // Do khác chuẩn nên phải thêm cho đúng chuẩn (Phía trên bó tay không thê đổi)
                                    userid: req.user.id,
                                });
                            } else {
                                res.send('Trang không tồn tại');
