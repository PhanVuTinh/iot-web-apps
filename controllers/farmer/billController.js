        ...
        if (error) {
            ...
            if (result.Error) {
                ...
                if (params.stocking_id) {
                    ...
                    if (flashCheckExes) {
                        res.render('farmer/bill/billList', {
                            title: 'Hóa đơn đợt thả nuôi số ' + params.stocking_id,
                            conf: config.urladdress,
                            token: token,
                            userId: req.user.id,
                            fullName: req.session.fullName,
                    ...
                    if (stockingOfUser.length != 0) {
                        res.render('farmer/bill/billList', {
                            title: 'Hóa đơn đợt thả nuôi số ' + stockingOfUser[0].stocking_id,
                            conf: config.urladdress,
                            token: token,
                            userId: req.user.id,
                            fullName: req.session.fullName,
        ...
        if (error) {
            ...
            if (result.Error) {
                ...
                if (req.params.stocking_id) {
                    ...
                    if (flashCheckExes) {
                        res.render('farmer/bill/addBill', {
                            title: 'Thêm hóa đơn cho đợt thả nuôi số ' + req.params.stocking_id,
                            conf: config.urladdress,
                            token: token,
                            userId: req.user.id,
                            fullName: req.session.fullName,
        ...
        if (error) {
            ...
            if (result.Error) {
                ...
                if (result.data.user_id == req.user.id) {
                    res.render('farmer/bill/updateBill', {
                        title: 'Cập nhật hóa đơn số ' + result.data.bill_id,
                        conf: config.urladdress,
                        token: token,
                        userId: req.user.id,
                        fullName: req.session.fullName,
