...
stokingController.get('/them', service.ensureAuthenticated, function(req, res) {
    ...
    res.render('farmer/stocking/createStocking', {
        title: 'Thêm đợt thả nuôi',
...
stokingController.get('/danhsach', service.ensureAuthenticated, function(req, res) {
    var token = "JWT " + req.session.token; // Token de goi API
    res.render('farmer/stocking/stockingList', {
        title: 'Quản lý thả nuôi',
        conf: config.urladdress,
        token: token,
        userId: req.user.id,
        fullName: req.session.fullName,
                    ...
                    service.get(options, function(error, result) {
                        if (error) {
                            ...
                            if (result.Error) {
                                ...
                                if (result.data != null) {
                                    res.render('farmer/stocking/detailStocking', {
                                        title: 'Đợt thả nuôi số ' + req.params.stocking_id,
                                        conf: config.urladdress,
                                        token: token,
                                        userId: req.user.id,
                                        fullName: req.session.fullName,
