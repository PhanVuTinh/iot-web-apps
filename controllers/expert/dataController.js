...
dataController.get("/xemdulieu",service.ensureAuthenticated, function(req, res) {
  res.render("expert/dulieu/xemdulieu", {title: 'Xem dữ liệu',
  secu:config.securitycode,
  conf:config.urladdress,token:req.session.token,userid:req.session.userid,username:req.session.username,fullName:req.session.fullName});
});
